...
- **nostromo** (Zen kernel workstation, tailnet `100.107.103.76`, LAN `192.168.254.97`): Hyprland desktop, Steam, MO2, ai/ollama, custom game servers via [[hangar]] (KF2, Vintage Story, Tarkov-SPT). Deploys via the `rebuild` alias (nh wrapper). Do NOT tell the user `colmena apply` for this host as it is the localhost.
...
- **nostromo**: `rebuild` (nh wrapper alias). Local system only.
- **ishimura** or **normandy**: colmena. Always commit first (colmena refuses dirty tree). `colmena apply --on <host>`.
- Commit style is **one line only, no body**. Skip Claude co-author trailers, skip generated-with footers.
- SSH to remote hosts uses port **36475** (port 22 is endlessh tarpit that will hang).
...
- **No em-dashes.** Ever. Use `-` (dash), `:`, `,`, or parentheses.
...
- `2026-07-09-RESUME-here.md` - open work batch (jellyfin-frontend, synctube, wiki, page-archiver, recipe-manager, intercom, small-utilities + extensions)
...
5. **Commit + deploy in one instruction.** When you finish an edit, give the user both the commit message (one line, no body) and the deploy command in the same message. For ishimura/normandy, remind them to commit first. For nostromo, just `rebuild`.
